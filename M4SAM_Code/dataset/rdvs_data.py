...
class RDVSDataset_vid(data.Dataset):
    ...
    ## 参考DViSal的结构
    '''
    def __init__(self, data_root, mode, augmentation, interval, sample_rate, trainsize, baseMode, video_length=5, frame_rate=25,drop_last=False,dont_trans_gt=False):        #RDVS没有subset，直接读取目录
        ...
        if mode == 'train':
            videolists = sorted(os.listdir(os.path.join(data_root, 'train')))
        elif mode == 'test':
            videolists = sorted(os.listdir(os.path.join(data_root, 'test')))
        else:
        ...
        for video in videolists:
            #一律按照标注帧来取
            gt_path = os.path.join(data_root, mode, video, 'ground-truth')#GT文件夹
            #取文件夹内所有标注帧的绝对路径
