...
class Mem(nn.Module):
    def __init__(self, config, model_path=None, map_location=None):
        ...
        #config['key_dim']        config['value_dim']         config['hidden_dim']
        self.single_object = config.get('single_object', True)
        
        #print(f'Single object mode: {self.single_object}')

        # self.key_encoder = key_encoder
        # key encoder在外部xmem4sam被重载
        #self.value_encoder = ValueEncoder(self.value_dim, self.hidden_dim, self.single_object)
        # value encoder在外部xmem4sam被重载

        # Projection from f16 feature space to key/value space
        self.key_proj = KeyProjection(self.key_dim,self.key_dim)
        '''原vit encoder的输出位256维度，现修改为128
        in_dim256, self.key_dim)'''

        self.decoder = MemoryDecoder(self.value_dim, self.hidden_dim,self.key_dim)

        self.reinforce = config['reinforce']
        if self.reinforce:
            #self.memory_reinforce = ForegroundReinforcingModule(256,128,256,3)
            self.memory_reinforce = ForegroundReinforcingModule(128,64,128,3)
    ...
    def encode_value(self, frame, image_feat_f16, h16, masks, is_deep_update=True): #弃用！
        num_objects = 1#采用单目标，没有多成mask输入
        if num_objects != 1:
            others = torch.cat([
                torch.sum(
    ...
    def encode_value2(self, frame, embedding, h16, masks, is_deep_update=True): 
        ...
        #memory_value    : B * num_objects * ChannelValue * T * H * W
        return g16.unsqueeze(3), h16
        #value(0),hidden  
    ...
    def init_hyperparameters(self, config, model_path=None, map_location=None):
        ...
        if model_path is not None:
            ...
            self.feat_dim = model_weights['key_encoder.encoder.0.weight'].shape[1]
            ...
            #添加了feat_dim
            if 'feat_dim' not in config:
                self.feat_dim = 128
                print(f'feat_dim not found in config. Set to default {self.feat_dim}')
            else:
