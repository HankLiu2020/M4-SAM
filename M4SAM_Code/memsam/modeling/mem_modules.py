...
class ValueEncoder(nn.Module):
    def __init__(self, value_dim, hidden_dim, single_object=True):
        super().__init__()
        ...
        self.rfb1 = RFB_modified(512, 64)#为11x11的512通道增加了一个rfb到64通道
        ...
        self.distributor = MainToGroupDistributor()#concat
        #self.fuser = FeatureFusionBlock(128, 256, value_dim, value_dim)
        #目前image_feat 128,g 64
        self.fuser = FeatureFusionBlock(64, 64, value_dim, value_dim)
        if hidden_dim > 0:
    ...
    def forward(self, image, image_feat, h, masks, others, is_deep_update=True):
        # image_feat_f16 is the feature from the key encoder:就是imge
        # 默认单目标
        '''if not self.single_object:
            g = torch.stack([masks, others], 2)
        else:
            g = masks.unsqueeze(1)#bx1xhxw'''
        g = self.distributor(image, masks.unsqueeze(1))    #  将image和g进行分发,按照g的shape进行分发

        batch_size, num_objects = g.shape[:2]
        g = g.flatten(start_dim=0, end_dim=1)#(bxn)
        ...
        g = self.layer1(g) # 1/4    64 88x88
        
        if image_feat.shape[-1]==44:
            g = self.layer2(g) # 1/8    128c 44x44
            conv1x1 = nn.Conv2d(128, 64, kernel_size=1, stride=1, padding=0).cuda()
            g = conv1x1(g)  # (batch_size, 64, 44, 44)
    
        if image_feat.shape[-1]==11:
            g = self.layer2(g) # 1/8    128c 44x44 为了对齐imge的64channel
            g = self.layer3(g) # 1/16   256  22x22
            g = self.layer4(g) # 1/32   512   11x11
            g = self.rfb1(g)   #         64   11x11
        g = g.view(batch_size, num_objects, *g.shape[1:])
...
class MemoryDecoder(nn.Module):
    def __init__(self, val_dim, hidden_dim,imge_dim=128):
        super().__init__()
        '''
        imge:64
        value_dim: 256#sam embedding:128
        hidden_dim: 64
        '''
        #self.fuser = FeatureFusionBlock(imge_dim, val_dim+hidden_dim, 512, 256)
        self.fuser = FeatureFusionBlock(imge_dim, val_dim+hidden_dim, 512, val_dim)
        if hidden_dim > 0:
